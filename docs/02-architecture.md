...
│       └── .ghost-app.pem        # GitHub App key (mode 0400, Ghost-only)
└── sandbox/
    └── <architect>/
...
> **TOS does not create or destroy remotes.** Repository creation and deletion
> are Architect operations, performed with the Architect's own tools. The
> Ghost's GitHub App is granted no `administration` permission, so it lacks
> the capability rather than merely declining to use it. Everything else is
> unchanged: `sync project` attaches a sandbox to a repository that already
> exists, and `close project` removes the sandbox while leaving the remote
> alone. See governance/adr.md § TOS Does Not Create or Destroy Remotes.

**`create`** — Inception. Opening a Trinity or an issue. Actions: `trinity`, `issue`.

**`sync`** — Alignment. Workspace lifecycle management. Getting into a working context, checking out branches, generating the Clean Room Snapshot. Actions: `project`, `trinity`, `peek`.
...
**`delete`** — Purge. Currently no actions. `delete project` was retired when the Ghost lost the capability to destroy a repository, and nothing else has needed this tier. The phase is retained because a future purge operation would belong here.
...
**Stage 2 — Configuration & Credential Injection.** Sources the global config, then mints a GitHub App installation token from the `0400` private key at `.local/conf/.ghost-app.pem`. The token is injected as a process-scoped git credential helper through `GIT_CONFIG_*` environment variables, alongside `GIT_CONFIG_NOSYSTEM=1`, `GIT_CONFIG_GLOBAL=/dev/null` and an emptied `credential.helper`, so git cannot fall back to any persistent store. Registers an `EXIT/INT/TERM` trap that scrubs the token, both token variables and every `GIT_CONFIG_*` entry, and cleans all temporary directories.

There is no token file. Installation tokens expire after an hour, so one is minted per invocation and never written to disk, a config file, a remote URL or a log — see governance/adr.md § Ephemeral Credential Lifecycle.
