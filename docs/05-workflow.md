...
- A GitHub App has been created for the Ghost, installed on the repositories TOS will manage, and its private key placed at `${TOS_MNT_ROOT}/.local/conf/.ghost-app.pem`. See docs/06-security.md § The Ghost Identity.
- The `gh` CLI is installed and authenticated.

---

## Phase 0 — Establishing Remote Truth (the Architect, not TOS)

**TOS never creates a repository.** The Ghost's GitHub App is granted `contents`, `issues` and `pull_requests`, and pointedly not `administration` — so it lacks the capability rather than declining to use it. An engine an LLM helps drive should not be able to reshape your account.

So the first move is yours, with your own credentials:

```zsh
gh repo create myproject --private --add-readme
gh repo clone myproject && cd myproject
```

`--add-readme` matters. Without it the repository is created with no commits
and no default branch, and TOS has nothing to attach to — the failure surfaces
later, at `create trinity`, as an error about a `main` branch you never made.
If you already have an empty repository, push any first commit to `main`.

Make sure the Ghost's App installation covers the new repository. On an "all repositories" installation this is automatic; on a selected-repositories installation you must add it, or the Ghost will not be able to see it.

Only then does TOS get involved, in Phase 1.
...
> **Push any shared code to `main` before the Trinity is opened.** A Trinity
> branches from `main` as it stands at that moment, and `sync trinity` pulls
> only the Trinity's own branch — it never fast-forwards from `main`. So a
> commit landing on `main` after the Trinity opens is invisible inside it:
> `sync peek` will not find the file, and the Agent reasons against a base that
> has quietly diverged.
>
> For a Trinity that lives an hour this is unremarkable. For a longer one, or a
> project with more than one Architect, the only workaround is "do not let
> `main` move" — which is a constraint, not a workflow. Whether a Trinity
> should absorb `main` mid-flight, and whether the Agent must be told when it
> does, is open. See the roadmap.
...
## Phase 12 — Deleting the Repository (the Architect, not TOS)

There is no `delete project`. It existed once and was retired with `create project`, for the same reason: destroying a repository is an Architect decision with consequences no agent should be able to trigger.

The Ghost's App holds no `administration` permission, so the capability is absent rather than withheld. There is no scope to escalate and no MFA step to enforce, because there is nothing to enforce it against.

If you want the repository gone, delete it yourself:

```zsh
gh repo delete myowner/myproject
```

Detach TOS first with `close project` (Phase 11) so no sandbox or lock is left pointing at something that no longer exists.
