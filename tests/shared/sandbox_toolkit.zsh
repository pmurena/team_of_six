...
sandbox_provision() {
    ...
    # See scaffold.zsh: the gateway requires a GitHub App with no fallback,
    # so the harness installs a stub minter rather than reaching the network.
    export TOS_APP_PEM="${TEST_MNT_ROOT}/.local/conf/.ghost-app.pem"
    echo "not-a-real-key" > "${TOS_APP_PEM}"
    chmod 0400 "${TOS_APP_PEM}"
    export TOS_GHOST_LOGIN="ghost-bot"
    cat > "${TOS_BIN}/utils/mint_app_token.zsh" <<'STUB'
#!/bin/zsh
# stub minter
echo "FAKE_GH_TOKEN"
STUB
