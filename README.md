...
| [06-security.md](docs/06-security.md) | The trust model, the Ghost's GitHub App identity, capability rather than restraint, the Out-of-Band Recovery Doctrine, and the control plane layout |
...
- **The Architect** — the human developer. Owns the repository, reviews all work, approves Intent Locks and phase transitions, and makes every final decision. The system is only as reliable as the Architect is skeptical.
- **The Agent** — the language model. Reasons, and writes structured plaintext payloads. It has no execution capability of any kind: not restricted, absent. Explicitly untrusted.
- **The Ghost** — the engine. A locked-down Unix user with its own sandbox and its own GitHub identity, which executes what the Gateway has already validated. It does not think, and it acts only on payloads the Architect has released to it.
- **The Gateway** — the enforcement layer. Validates every operation before the Ghost executes it. Enforces lock tiers, manifest visas, hallucination checks, phase transitions and destructive gates. Cannot be bypassed by any role.

The separation between the Agent and the Ghost is the one the whole
architecture rests on: the thing that reasons and the thing that executes are
different, and neither can become the other. Collapsing them in prose is how
that guarantee starts to erode.
