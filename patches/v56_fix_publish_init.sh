#!/bin/bash
# Patch Name: V56 Fix Publish Init Order
# Location: patches/v56_fix_publish_init.sh
# Purpose: Ensure 'git init' runs before 'git config' in tos_publish.sh.

PATCH_DIR="$(cd "$(dirname "$0")" && pwd)"
BIN_DIR="$(cd "$PATCH_DIR/../bin" && pwd)"

echo "🔧 Patching tos_publish.sh execution order..."
...
chmod +x "$BIN_DIR/tos_publish.sh"
echo "✅ Patch applied. 'team_of_six publish' should now function correctly."
