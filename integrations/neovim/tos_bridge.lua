...
vim.api.nvim_create_user_command("TosWork", function()
    ...
    local context_path = "/run/team_of_six/" .. user .. "_work_context.md"
    local file = io.open(context_path, "r")
    if not file then return vim.notify("❌ No context found. Run 'tos <project> work <id>'", vim.log.levels.ERROR) end
...
local function run_tos_selection()
    ...
    local ipc_file = "/run/team_of_six/" .. (os.getenv("USER") or "architect") .. "_input.sh"
    
    local f = io.open(ipc_file, "w")
    if not f then return print("❌ Error writing to IPC buffer") end
