...
# --- 0. FETCH ----------------------------------------------------------------
# Before anything else, because "is there work to preserve?" cannot be
# answered without knowing what the remote already has.
...
# --- 1. PRESERVE -------------------------------------------------------------
# The Ghost’s work must reach the remote before any local state changes. But
# only if there IS any: after an Architect review the sandbox is behind the
# remote with nothing of its own to preserve, and pushing then is both
# pointless and fatal (non-fast-forward).
#
#   ahead           the Ghost has commits the remote lacks   → push
#   behind only     the Architect reviewed                   → nothing to do
#   both            genuinely divergent histories            → halt
if [[ -n "$TOS_ACTIVE_TRINITY" && "$TOS_ACTIVE_TRINITY" -gt 0 ]]; then
    ...
    if git show-ref --verify --quiet "refs/heads/$OUTGOING"; then
        if git show-ref --verify --quiet "refs/remotes/origin/$OUTGOING"; then
            AHEAD=$(git rev-list --count "origin/${OUTGOING}..${OUTGOING}" 2>/dev/null || echo 0)
            BEHIND=$(git rev-list --count "${OUTGOING}..origin/${OUTGOING}" 2>/dev/null || echo 0)
        else
            AHEAD=$(git rev-list --count "$OUTGOING" 2>/dev/null || echo 0)
            BEHIND=0
        fi

        if (( AHEAD > 0 && BEHIND > 0 )); then
            echo "🚨 FATAL: $OUTGOING has diverged from origin — $AHEAD local, $BEHIND remote." >&2
            echo "    Two histories claim the same branch. This is the case the" >&2
            echo "    Out-of-Band Recovery Doctrine reserves for you: resolve it on" >&2
            echo "    the remote, then re-run this command." >&2
            exit 1
        elif (( AHEAD > 0 )); then
            echo "⬆️  Preserving $OUTGOING on remote ($AHEAD commit(s))..."
            ...
            echo "⏭️  Nothing to preserve — $BEHIND commit(s) behind origin, absorbed below."
        else
            echo "⏭️  Nothing to preserve — $OUTGOING matches origin."
        fi
    else
...
if [[ "$NEW_TRINITY" != "0" ]]; then
    ...
    # --json, not the human view. `gh issue view --comments` writes NOTHING when
    # stdout is not a terminal and still exits 0, so the fallback below never
    # fired and the Required Context was silently blank in every snapshot this
    # project has ever produced.
    if ! gh issue view "$NEW_TRINITY" \
...
{{.body}}
{{end}}' 2>/dev/null; then
        echo "_(could not read issue #$NEW_TRINITY — does it exist?)_"
    fi

    echo ""
    echo "## PULL REQUEST"
    if ! gh pr view "$TARGET_BRANCH" \
            --json number,title,state,isDraft,body,comments,reviews \
            --template '#{{.number}} {{.title}} [{{.state}}{{if .isDraft}}, draft{{end}}]
{{.body}}
{{range .reviews}}
