...
# ------------------------------------------------------- GitHub fetch -------
REPO_FULL=$("${TOS_BIN}/utils/resolve_repo.zsh" "$PROJECT") \
    || _die "Could not resolve '${PROJECT}' through the App installation."
...
# A GitHub App installation token has no user behind it: `gh api user` returns
# 403. The login is therefore declared in conf/config rather than discovered.
GHOST_LOGIN="${TOS_GHOST_LOGIN:-}"
[[ -z "$GHOST_LOGIN" ]] && GHOST_LOGIN=$(gh api user -q .login 2>/dev/null)
[[ -n "$GHOST_LOGIN" ]] || _die "Could not resolve the Ghost's GitHub login.
    Set TOS_GHOST_LOGIN in conf/config. For a GitHub App it is <slug>[bot]."
